...
namespace pcpp
{
	...
	/**
	 * @class HinfoDnsResourceData
	 * A class that represents DNS HIFO data, used in DNS RRs of type ::DNS_TYPE_SRV
	 */
	class HinfoDnsResourceData : public IDnsResourceData
	{
	public:
		...
		struct HinfoData
		{
			uint8_t cpulen; /* len of cpu */
			std::string cpu;
			uint8_t oslen; /* len of os */
			std::string os;
		};

		/**
		 * A c'tor for this class
		 * @param[in] dataPtr A byte array that contains the raw Hinfo data (as written in the DNS packet)
		 * @param[in] dataLen The byte array size
		 * @param[in] dnsResource A pointer to a DNS resource object where this DNS RR data will be stored
		 */
		HinfoDnsResourceData(uint8_t* dataPtr, size_t dataLen, IDnsResource* dnsResource);

		/**
		 * A c'tor for this class
		 * @param[in] preference The Hinfo preference value to store in this object
		 * @param[in] mailExchange The Hinfo hostname value to store in this object. It's possible to include a pointer to
		 * another string in the DNS layer (as explained here: http://www.zytrax.com/books/dns/ch15/#name). These pointers
		 * are often used to reduce the DNS packet size and avoid unnecessary duplications. The way to include pointers
		 * in the hostname string is to use the following format: 'some.domain.#{offset}' where '#{offset}' is the offset
		 * from the start of the DNS layer. For example: if the string 'yahoo.com' already appears in offset 12 in the
		 ...
		 * 'my.subdomain.#12'. This will result in writing 'my.subdomain' and a pointer to offset 12
		 */
		HinfoDnsResourceData(const uint8_t& cpulen, const uint8_t& oslen, const std::string& cpu, const std::string& os);

		~HinfoDnsResourceData() {}

		/**
		 * Equality operator overload for this class that compares the Hinfo data stored in each object
		 * @param[in] other The object to compare with
		 * @return True if Hinfo data is the same in both objects, meaning both preference and MX hostname are the same,
		 * false otherwise
		 */
		bool operator==(const HinfoDnsResourceData& other) const;

		/**
		 * @return The Hinfo data stored in this object
		 */
		HinfoData getHinfoData() const { return m_Data; }

		/**
		 * Set the Hinfo data stored in this object
		 * @param[in] preference The MX preference value to store in this object
		 * @param[in] mailExchange The MX hostname value to store in this object
		 */
		void setHinfoData(uint8_t cpulen, uint8_t oslen, std::string cpu, std::string os);

		// implement abstract methods

		/**
		 * A string representation of the Hinfo data stored in this object. The string format is as follows:
		 * 'pref: {preference_value}; mx: {mail_exchange_hostname_value}'
		 */
		std::string toString() const;
